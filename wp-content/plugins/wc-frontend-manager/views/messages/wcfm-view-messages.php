		...
		<?php if( apply_filters( 'wcfm_is_allow_direct_message', true ) && apply_filters( 'wcfm_is_pref_direct_message', true ) ) { ?>
			<?php if( !wcfm_is_vendor() || ( wcfm_is_vendor() && WCFM_Dependencies::wcfmu_plugin_active_check() ) ) { ?>
				<form id="wcfm_messages_form" class="wcfm">
				  <div class="wcfm-container wcfm-top-element-container">
						<h2><span class="fa fa-send-o"></span>&nbsp;<?php _e('Send Direct Message', 'wc-frontend-manager' ); ?>&nbsp;-&nbsp;<?php if( wcfm_is_vendor() ) { _e('To Store Admin', 'wc-frontend-manager' ); } else { _e('To Store Vendors', 'wc-frontend-manager' ); } ?></h2>
						<div class="wcfm-clearfix"></div>
					</div>
					<div class="wcfm-clearfix"></div><br />
					<div class="wcfm-container">
						<div id="wcfm_messages_listing_expander" class="wcfm-content">
							...
							$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_messages_field_users', array(
																																																			"wcfm_messages" => array( 'type' => $wpeditor, 'class' => 'wcfm-textarea wcfm_ele ' . $rich_editor, 'label_class' => 'wcfm_title', 'value' => $wcfm_messages ),
																																																			) ) );
							?>
