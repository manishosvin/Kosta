...
$current_order_ststus = apply_filters( 'wcfm_current_order_status', $order->get_status(), $order->get_id() );

$status_update_block_statuses = apply_filters( 'wcfm_status_update_block_statuses', array( 'refunded', 'cancelled', 'failed' ) );
  ...
  <div class="wcfm-page-headig">
	...
	<div class="wcfm-collapse-content">
		...
		<div class="wcfm-container wcfm-top-element-container">
			...
			<span class="order-status order-status-<?php echo sanitize_title( $current_order_ststus ); ?>"><?php _e( ucfirst(wc_get_order_status_name( $current_order_ststus )), 'wc-multivendor-marketplace' ); ?></span>
			
			<?php
			if( $allow_wp_admin_view = apply_filters( 'wcfm_allow_wp_admin_view', true ) ) {
				?>
				<a target="_blank" class="wcfm_wp_admin_view text_tip" href="<?php echo admin_url('post.php?post='.$order_id.'&action=edit'); ?>" data-tip="<?php _e( 'WP Admin View', 'wc-frontend-manager' ); ?>"><span class="fa fab fa-wordpress fa-wordpress-simple"></span></a>
				<?php
			}
			?>
				...
				<?php if( apply_filters( 'wcfm_is_allow_order_status_update', true ) ) { ?>
					<div id="wcfm_order_status_update_wrapper" class="wcfm_order_status_update_wrapper">
						<p class="form-field form-field-wide wc-order-status">
							...
							<?php
							  if( !in_array( $current_order_ststus, $status_update_block_statuses ) ) {
				...
				<table cellpadding="0" cellspacing="0" class="woocommerce_order_items">
					...
					<?php
						foreach ( $line_items as $item_id => $item ) {
							...
							if( current_user_can( 'edit_published_products' ) && apply_filters( 'wcfm_is_allow_edit_products', true ) && apply_filters( 'wcfm_is_allow_edit_specific_products', true, $item->get_product_id() ) && apply_filters( 'wcfm_is_allow_order_details_product_permalink', true ) ) {
					...
					<?php if( apply_filters( 'wcfm_order_details_fee_line_item', true ) ) { ?>
					...
					<?php if( apply_filters( 'wcfm_order_details_refund_line_item', true ) ) { ?>
