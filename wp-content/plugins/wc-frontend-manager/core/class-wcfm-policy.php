...
class WCFM_Policy {
	...
	public function __construct() {
		...
		add_action( 'end_wcfm_products_manage', array( &$this, 'wcfm_policy_product_settings' ), 480 );
	...
	function wcfm_policy_settings_update( $wcfm_settings_form ) {
		...
		update_option( 'wcfm_policy_options', $wcfm_policy_options );
	}
	...
	function wcfm_policy_vendor_settings( $vendor_id ) {
		...
		$wcfm_policy_vendor_options = (array) get_user_meta( $vendor_id, 'wcfm_policy_vendor_options', true );
	...
	function wcfm_policy_product_settings( $product_id ) {
		global $WCFM;
		
		if( !apply_filters( 'wcmp_vendor_can_overwrite_policies', true ) || !apply_filters( 'wcfm_is_allow_policy_settings', true ) || !apply_filters( 'wcfm_is_allow_policy_product_settings', true ) ) return; 
		...
		<div class="page_collapsible products_manage_policies simple variable grouped external booking" id="wcfm_products_manage_form_policies_head"><label class="fa fa-ambulance"></label><?php _e('Product Policies', 'wc-frontend-manager'); ?><span></span></div>
		<div class="wcfm-container simple variable external grouped booking">
			<div id="wcfm_products_manage_form_policies_expander" class="wcfm-content">
				...
				$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'product_manage_fields_policies', array(  
