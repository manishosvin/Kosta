...
class WCFM_Enquiry {
   ...
   */
	function wcfmmp_store_enquiry_button() {
		...
		if( apply_filters( 'wcfm_is_pref_enquiry_button', true ) ) {
			...
			?>
			<div class="lft bd_icon_box"><a class="wcfm_store_enquiry" data-store="<?php echo $vendor_id; ?>" data-product="0" href="#"><div class="bd_icon"><i class="fa fa-question" aria-hidden="true"></i></div><span><?php _e( 'Inquiry', 'wc-frontend-manager' ); ?></span></a></div>
			<?php
		}
	}
   ...
   */
	function wcfmmp_store_list_enquiry_button( $store_id ) {
		...
		if( apply_filters( 'wcfm_is_pref_enquiry_button', true ) && apply_filters( 'wcfm_is_allow_store_list_enquiry', true ) ) {
			?>
			<p class="store-enquiry"><a class="wcfm_catalog_enquiry" data-store="<?php echo $store_id; ?>" data-product="0" href="#"><span class="fa fa-question-circle fa-question-circle-o"></span>&nbsp;<span class="add_enquiry_label"><?php _e( 'Inquiry', 'wc-frontend-manager' ); ?></span></a></p>
			<?php
		}
	}
   ...
   */
	...
	function wcfm_enquiry_scripts() {
 		...
 		if( is_product() || ( function_exists( 'wcfmmp_is_store_page' ) && ( wcfmmp_is_store_page() || wcfmmp_is_stores_list_page() ) ) ) {
 	...
 	function wcfm_enquiry_styles() {
 		...
 		if( function_exists( 'wcfmmp_is_store_page' ) && ( wcfmmp_is_store_page() || wcfmmp_is_stores_list_page() ) ) {
 			wp_enqueue_style( 'wcfm_enquiry_button_css',  $WCFM->library->css_lib_url_min . 'enquiry/wcfm-style-enquiry-button.css', array(), $WCFM->version );
 		}
