  ...
  function wcpvendors_after_line_total( $item, $order ) {
  	...
  	if( $item->get_product_id() ) {
		...
		if( !empty( $order_line_due ) ) {
			$line_total = floatval($order_line_due[0]->product_amount);
		  $line_total += floatval($order_line_due[0]->product_shipping_amount); 
		  $line_total += floatval($order_line_due[0]->product_tax_amount);
		  $line_total += floatval($order_line_due[0]->product_shipping_tax_amount);
		?>
