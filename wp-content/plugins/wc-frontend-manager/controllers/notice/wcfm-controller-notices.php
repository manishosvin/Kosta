		...
		if(!empty($wcfm_notices_array)) {
			...
			foreach($wcfm_notices_array as $wcfm_notices_single) {
				...
				if( current_user_can('administrator') ) {
					...
					if( $wcfm_notices_single->post_status != 'publish' ) {
						$actions .= '<a class="wcfm_notice_publish wcfm-action-icon" href="#" data-noticeid="' . $wcfm_notices_single->ID . '"><span class="fa fa-check-square-o text_tip" data-tip="' . esc_attr__( 'Publish - on line this now', 'wc-frontend-manager' ) . '"></span></a>';
					} else {
						$actions .= '<a class="wcfm_notice_archive wcfm-action-icon" href="#" data-noticeid="' . $wcfm_notices_single->ID . '"><span class="fa fa-times-circle text_tip" data-tip="' . esc_attr__( 'Archive - off line for the timing', 'wc-frontend-manager' ) . '"></span></a>';
					}
