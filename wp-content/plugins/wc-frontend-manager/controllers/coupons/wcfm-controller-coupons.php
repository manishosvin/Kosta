	...
	public function processing() {
		...
		if(!empty($wcfm_coupons_array)) {
			...
			foreach($wcfm_coupons_array as $wcfm_coupons_single) {
				...
				// Store
				if( !wcfm_is_vendor() ) {
					$coupon_author = $wcfm_coupons_single->post_author;
					if( wcfm_is_vendor( $coupon_author ) ) {
						$wcfm_coupons_json_arr[$index][] = $WCFM->wcfm_vendor_support->wcfm_get_vendor_store_by_vendor( $coupon_author );
					} else {
