...
class WCFM_Vendors_Controller {
	...
	public function processing() {
		...
		if( isset($_POST['filter_date_form']) && !empty($_POST['filter_date_form']) ) {
			$filter_date_form = $_POST['filter_date_form'];
		}
		...
		if(!empty($wcfm_vendors_array)) {
			...
			foreach($wcfm_vendors_array as $wcfm_vendors_id => $wcfm_vendors_name ) {
				...
				if( $is_store_offline ) {
					$wcfm_vendors_json_arr[$index][] = '<span class="order-status tips fa fa-power-off text_tip" style="color: #ff9310;" data-tip="' . __( 'Store Off-line', 'wc-frontend-manager' ) . '"></span>';
				} else {
					if( $disable_vendor ) {
				...
				if( apply_filters( 'wcfm_is_allow_manage_vendor', true ) ) {
					...
					$vendor_display_name = '<span class="wcfm_dashboard_item_title">' . apply_filters( 'wcfm_vednors_display_name_data', $wcfm_vendors_name, $wcfm_vendors_id ) . '</span>';
				}
				...
				$wcfm_vendors_json_arr[$index][] = apply_filters( 'wcfm_vednors_earned_commission_data', wc_price( $earned ), $wcfm_vendors_id, $report_for, $filter_date_form, $filter_date_to );
				...
				$wcfm_vendors_json_arr[$index][] = apply_filters( 'wcfm_vednors_received_commission_data', wc_price( $received_commission ), $wcfm_vendors_id, $report_for, $filter_date_form, $filter_date_to );
