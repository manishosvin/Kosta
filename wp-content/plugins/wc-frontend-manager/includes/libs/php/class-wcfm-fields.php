...
class WCFM_Fields {
  ...
  public function checkbox_input($field) {
    ...
    printf(
        '<input type="checkbox" id="%s" name="%s" class="%s" value="%s" %s %s %s %s />',
        esc_attr($field['id']),
        esc_attr($field['name']),
        esc_attr($field['class']),
        esc_attr($field['value']),
  ...
  public function radio_input($field) {
    ...
    $options = '';
    foreach ( $field['options'] as $key => $value ) {
      $options .= '<label title="' . esc_attr($key) .'"><input class="' . esc_attr($field['class']) . '" type="radio" ' . checked( esc_attr($field['value']), esc_attr($key), false ) . implode( ' ', $custom_attributes ) . '  value="' . esc_attr($key) . '" name="' . esc_attr($field['name']) . '"> <span>' . esc_html($value) . '</span></label><br />';
    }
  ...
  public function upload_input($field) {
    ...
    printf(
        ...
        esc_attr($field['id']),
        $field['mime'],
        __( 'Upload', 'wc-frontend-manager' ),
        esc_attr($field['id']),
        esc_attr($field['id']),
        $field['mime'],
        __( 'Remove', 'wc-frontend-manager' )
    );
  ...
  public function colorpicker_input($field) {
    ...
    printf(
        '<input type="%s" id="%s" name="%s" class="%s" data-default="%s" value="%s" %s />',
        $field['type'],
        esc_attr($field['id']),
        esc_attr($field['name']),
        esc_attr($field['class']),
  ...
  public function datepicker_input($field) {
    ...
    /**
   * Output a html show box.
   *
   * @access public
   * @param array $field
   * @return void
   */
  public function html_input($field) {
    $field['value'] 		= isset( $field['value'] ) ? $field['value'] : '';
    $field['name'] 			= isset( $field['name'] ) ? $field['name'] : $field['id'];
    $field['class'] 		= isset( $field['class'] ) ? $field['class'] : '';
    ...
    printf(
        '<div id="%s" name="%s" class="%s" %s %s>%s</div>',
        esc_attr($field['id']),
        esc_attr($field['name']),
        esc_attr($field['class']),
        implode( ' ', $custom_attributes ),
  ...
  public function field_wrapper_start($field) {
    ...
    if(isset($field['label'])) {
      ...
      if( isset( $field['hints'] ) && !empty( $field['hints'] ) ) {
        printf(
          '<span class="img_tip fa fa-question fa-question-circle-o" data-tip="%s"></span>',
