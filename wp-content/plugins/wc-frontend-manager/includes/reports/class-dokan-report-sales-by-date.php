...
class Dokan_Report_Sales_By_Date extends WC_Admin_Report {
	...
	private function query_report_data() {
		...
		// Earned commission
		$this->report_data->total_earned = $WCFM->wcfm_vendor_support->wcfm_get_commission_by_vendor( $this->vendor_id, $this->current_range );
		
		// Withdrawal Commission
		$this->report_data->total_withdraw = $WCFM->wcfm_vendor_support->wcfm_get_commission_by_vendor( $this->vendor_id, $this->current_range, true );
	...
	public function get_chart_legend() {
		...
		if( apply_filters( 'wcfm_sales_report_is_allow_earning', true ) ) {
			$legend[] = array(
				...
				'color'            => $this->chart_colours['earned'],
				'highlight_series' => 3
			);
		}
		...
		if( apply_filters( 'wcfm_sales_report_is_allow_withdrawal', true ) ) {
			$legend[] = array(
				'title'            => sprintf( __( '%s total withdrawal', 'wc-frontend-manager' ), '<strong>' . wc_price( $data->total_withdraw ) . '</strong>' ),
				'placeholder'      => __( 'This is the sum of the commission withdraw including shipping and taxes if applicable.', 'wc-frontend-manager' ),
				'color'            => $this->chart_colours['withdraw'],
				'highlight_series' => 4
			);
		}
	...
	public function get_main_chart( $show_legend = 1 ) {
		...
		// Earned Commissions
		$select = "SELECT COALESCE( SUM( commission.net_amount ), 0 ) AS total_earned, p.post_date AS time";
		...
		$data['total_earned_commission'] = $this->prepare_chart_data( $results, 'time', 'total_earned', $this->chart_interval, $this->start_date, $this->chart_groupby );
		
		// Withdraw Commission
		$sql = "SELECT SUM( withdraw.amount ) AS withdraw, withdraw.date AS time FROM {$wpdb->prefix}dokan_withdraw AS withdraw";
		$sql .= " WHERE 1=1";
		$sql .= " AND withdraw.user_id = %d";
		$sql .= " AND withdraw.status = 1";
		$sql = wcfm_query_time_range_filter( $sql, 'date', $this->current_range, '', '', 'withdraw' );
		$sql .= " GROUP BY DATE( withdraw.date )";
		$results = $wpdb->get_results( $wpdb->prepare( $sql, $this->vendor_id ) );
		$data['total_paid_commission'] = $this->prepare_chart_data( $results, 'time', 'withdraw', $this->chart_interval, $this->start_date, $this->chart_groupby );
		...
		$chart_data = '{'
			...
			. ', "total_earned_commission"  : ' . $WCFM->wcfm_prepare_chart_data( $data['total_earned_commission'] )
			. ', "total_paid_commission"    : ' . $WCFM->wcfm_prepare_chart_data( $data['total_paid_commission'] )
